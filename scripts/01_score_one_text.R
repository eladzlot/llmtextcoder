...
result   <- score_one(template, list(text = script.ruminate), params)
scores <- result$scores

# --- downstream analysis: overall score (deterministic, no API) ------------
# Lives HERE, not in the engine, because the rule is project-specific.
overall <- round(mean(unlist(scores)))
cat("Subscores:\n"); print(scores)
cat("Overall (rounded mean):", overall, "\n")
